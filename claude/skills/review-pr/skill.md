...
プレフィックスの種別・用途は `implement-issue/skill.md`「12. コミット＆プッシュ」節のコミットメッセージ規約に従う。`improvement` ラベルの付与要否は以下の通り。

| プレフィックス | `improvement` ラベル |
|--------------|-------------------|
| `fix:` | 付与しない |
| `refactor:` | **付与する** |
| `test:` | **付与する** |
| `perf:` | 軽微な最適化は**付与する**、重大な性能問題は付与しない |
| `docs:` | **付与する** |
| `feat:` | 判断による（UX改善等は**付与する**） |
