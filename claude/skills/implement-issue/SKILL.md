...
実装を開始する**前に**過去の知見を把握する。**このステップは省略不可。**参照先は既定で `docs/DEVELOPMENT_NOTES.md` とするが、プロジェクトの `.claude/instructions.md` / `CLAUDE.md` に開発ノートの参照先定義（例: 設計書サイトを別リポジトリで運用している場合の参照先）があれば、そちらに従う。
...
## 5. 設計書の確認・更新（実装より前・必須）

実装に着手する**前**に、対象領域の設計書を確認し、今回の変更を反映するよう更新する。**このステップは省略不可。設計書更新を実装後に後回しにしてはならない。** 新規実装・改修・バグ修正のいずれであっても、設計が実装に先行することを原則とする。

- 参照先は既定でプロジェクト内の `docs/design/` 等とするが、プロジェクトの `.claude/instructions.md` / `CLAUDE.md` に設計書の参照先定義（例: 設計書サイトを別リポジトリで運用している場合の参照先）があれば、そちらに従う
- 変更する機能・仕様に対応する設計書（バッチ設計書・画面設計書・テスト設計書等）がまだ存在しない場合は新規作成する
- 既存の設計書がある場合は、今回の変更内容（新しい仕様・挙動・データ構造）を反映するよう更新する
- **バグ修正の場合:** ステップ2の原因調査結果をもとに、バグを誘発する記述や誤った前提が設計書にあれば修正する（例: 誤った手順・誤解を招く説明・古い動作を前提とした記述）
- 更新・作成した設計書の内容をユーザーに提示し、**承認を得てから**実装（次のステップ）に進む
- **設計書が実装対象リポジトリとは別のリポジトリで管理されている場合**: 該当リポジトリ側で先に更新・コミットし、PRを作成する。設計書の所在リポジトリが実装対象と同じか別かは着手順序に影響しない——別リポジトリであっても実装より前に完了させる。リポジトリをまたいだ `Closes` によるIssue自動クローズは信頼できないため使わず、実装側PRの `Follow-up` セクションに設計書側のPRへの言及を残すにとどめる
  - 実装PR作成（ステップ14）の時点で設計書側PRがまだマージされていない場合は、実装PR本文の**最も先頭**（`## Summary` より前）に「## レビュー前提条件」チェックリストを追加し、設計書側PRのマージを実装PRレビューの前提条件として明示する（配置理由はステップ14参照）
...
### 想定より規模が大きいと判明した場合の分割

実装を進める中で、Issue本文に明記されていない規模の追加対応（例: 型チェックを有効化したら大量の既存エラーが検出された、同じ修正が多数の既存ファイルに必要と判明した等）が必要と分かった場合、無理に一つのPRで全て対応しようとしない。

- 対応不要と判断した部分を後続Sub-Issueとして切り出す方針をユーザーに報告し、了承を得る
- 了承後、`gh issue create` で後続Issueを作成する。元Issueに親Issueがある場合は同じ親Issueの Sub-Issue として登録し、元Issueと兄弟関係にする
- 元PRの `Follow-up` セクション（ステップ14）に作成した後続Issue番号を明記する
- この分割は `think-issue` を介さずその場で行ってよい。`improvement` ラベルのIssueは think-issue の自動推奨対象外（`next-issue/SKILL.md` 参照）のため、実装中に判明したスコープ超過はこの分割が主な受け皿になる

### クライアント/サーバー境界の遵守（Next.js App Router）

`"use client"` コンポーネントは、サーバー専用モジュール（DB接続・ファイルシステム・サーバー側環境変数に依存するコード等）を直接・間接的に import してはならない。

- **新しい import を `"use client"` ファイルに追加するとき**: import 先のトップレベル import を1段確認し、サーバー専用依存が含まれていないかチェックすること
- **純粋関数・ユーティリティはサーバー専用ファイルと分離する**: どこでも使える関数をサーバー依存のあるファイルに混在させない。独立した `*-utils.ts` 等に切り出すこと
- この種の違反は `tsc`・`vitest`・`eslint` では検出されず、ブラウザ実行時のランタイムエラーとして初めて発覚する点に注意

## 7. テスト作成

### 7-1. 既存テストへの影響確認（必須・テスト実行前に行うこと）
...
`app/` または `components/` 配下の画面・UIロジックを追加・変更した場合、実装完了後に実際にブラウザで動作を確認する（対象範囲の判断基準はステップ11-aのFigma JSON更新と同じ）。スクリプト・ロジックのみの変更はスキップしてよい。

- dev server（`bun run dev`）を起動し、Playwright（`chromium-cli` があれば優先。なければ `node` から `playwright` パッケージを直接使うスクリプトで代用）で対象画面を操作する
- 新機能だけでなく、**変更が影響しうる既存モード・既存フローも壊れていないか**を合わせて確認する
- `page.on('console'/'pageerror')` 等でランタイムエラーが出ていないことを確認する
- 確認した画面状態ごとにスクリーンショットを撮り、一時ディレクトリ（`/tmp` 等）に保存しておく。ファイル・コミットへの格納はステップ14（PR作成後）で行う
- ここで実際に確認した操作・観点は、ステップ14のTest Planにそのまま反映する（確認していない項目を記載しない。確認した項目は必ず載せる）

### レスポンシブ（モバイル/デスクトップ）表示の確認

`@media` ブレークポイントを追加・変更した場合は、両方の幅を実際にレンダリングして比較する。`chromium.launch()` で1つのブラウザを起動し、幅ごとに `browser.newContext({ viewport: { width, height } })` で別コンテキストを作って同じ操作シーケンス（画面遷移・モード切替・データ選択等）を流す。1スクリプト内で `checkViewport(browser, viewport, label)` のような関数化をしておくと、幅を増やしても差分が出しやすい。
...
### 9-a. スクリプト・バッチ変更の動作確認

**変更したスクリプトに `--dry-run` が実装されている場合**、実データに対して `--dry-run` 実行することを既定の動作とする。省略する場合は理由を添えてユーザーに確認を取る。

**`--dry-run` が実装されていない場合**、実行すると書き込み・外部API呼び出しが発生するため、実行の要否をユーザーに確認する。
...
**知見がある場合:** 開発ノート（既定で `docs/DEVELOPMENT_NOTES.md`。プロジェクトの `.claude/instructions.md` / `CLAUDE.md` に別の参照先定義があればそちらに従う）に追記する。
...
## 11. Figma UI構造JSON更新・設計書との整合性最終確認

### 11-a. Figma UI構造JSON更新（UI変更がある場合）

画面・コンポーネントの**追加・変更・削除**があった場合は、`json-to-figma` スキルを使って `.figma/ui-structure.json` を更新し、コミットに含める。

- 呼び出し: `/json-to-figma` （引数不要。スキルが直近の変更ファイルを自律的に解析する）
- 更新対象の判断基準:
  - `app/` または `components/` 配下のファイルを新規作成・変更した場合
  - ページのレイアウト・セクション構成・主要コンポーネントが変わった場合
- UI変更がない（スクリプト・ロジックのみ）場合はスキップしてよい
- 更新後はコミット前に `.figma/ui-structure.json` を `git add` してコミットに含めること

### 11-b. 設計書との整合性の最終確認

ステップ5で更新・作成した設計書と、実際の実装内容に齟齬がないか最終確認する。実装中にステップ5の時点で想定していなかった変更が発生した場合は、ここで設計書側を追随して更新する。

- 本来は発生しないはずの手戻りである点に注意する。頻発する場合はステップ5での設計検討が不十分だったサインなので、ステップ10の開発ノート振り返りに記録する
- 設計書を別リポジトリ側で追加更新した場合は、ステップ5-aのビルド確認を再度実施する
...
### コミットメッセージのプレフィックス規約（Conventional Commits）

コミットメッセージの先頭には、変更内容に応じて以下のプレフィックスを付与する。

| プレフィックス | 用途 |
|--------------|------|
| `feat:` | 新機能・機能追加 |
| `fix:` | バグ修正 |
| `refactor:` | コード品質改善（動作を変えないリファクタリング） |
| `test:` | テストの追加・改善 |
| `perf:` | パフォーマンス改善 |
| `docs:` | ドキュメントの追加・修正 |
| `style:` | フォーマット・空白等、動作に影響しない見た目上の変更 |
| `chore:` | ビルド設定・依存関係更新等、上記に当てはまらない雑務的変更 |

`review-pr` スキルがレビュー指摘からIssueを作成する際のタイトルプレフィックスも、このプレフィックス種別を参照する（付与するIssueラベルの判断基準は `review-pr/skill.md` 側を参照）。

### 実装対象Issueのラベルとの整合

実装対象のIssueに `improvement` ラベルが付いている場合、コミットプレフィックスは `review-pr/skill.md` の対応表で `improvement` ラベルを**付与する**側（`refactor:` / `test:` / `docs:` / `style:` / `chore:`）から、差分の実態に最も合うものを選ぶ。`fix:`（付与しない対応）は用途が異なるため通常選ばない。

実装した内容が実態として新機能追加であり `feat:` が妥当と判断される場合は、プレフィックスを機械的に決めず、Issue自体のラベルが `improvement` のままで良いか（`enhancement` 等への変更が必要ないか）をユーザーに確認してから選択する。
...
```
## レビュー前提条件
- （ステップ5で設計書側PRがマージ待ちの場合のみ。無ければセクションごと省略）

## Summary
- （変更点を箇条書き）

## Screenshots
- （ステップ9でUI動作確認をした場合のみ。フォーマットは後述）
...
- **`## レビュー前提条件` は本文の最も先頭に置くこと**（`## Summary` より前）。レビュアーがマージ可否を判断する前提となるブロッキング情報のため、他のセクションより先に読める位置に置く
- Test plan に追加する動作確認項目は **Preview や手動操作で再現できるものに限定すること**。特定の日付・データが必要な確認など再現が難しい項目は追加しない。そのような動作はユニットテストでカバーすること
- **動作確認項目の書き方**:
  - UI操作で確認する項目: **「〈操作〉する → 〈期待結果〉こと」** の形式で書く。解釈のブレや観点漏れを防ぐため、何を操作して何が起きるかを1項目に明記すること
    - NG: `マップアイコンが表示されること`（何をしたら？が不明）
    - OK: `タイムライン画面でメンバーが1人以上いるイベントカードを確認する → マップアイコンが表示されること`
  - スクリプト・コマンドで確認する項目: コマンドをそのまま記載する（例: `` `bun run test` が全てパスすること ``）
  - URL直打ちはコマンドベースの表現になるため使わない。ユーザー操作（ナビゲーションクリック、リンク遷移など）に言い換えること
  - 画面遷移を伴う機能は、起点画面 → 操作 → 遷移先での確認 の流れを軸に項目を並べること
  - ステップ9でUI動作確認をした場合、そこで実際に操作・確認した内容をそのままTest plan項目に反映する（確認していないことを書かない。確認したことは漏らさず書く）
- ステップ8で実施済みの bun run test / lint / type-check は `[x]` でチェック済みにすること
...
### 14-a. スクリーンショットの格納とPRへのリンク（ステップ9でUI動作確認をした場合）
