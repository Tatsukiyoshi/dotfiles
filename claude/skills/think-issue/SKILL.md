...
| 分割軸 | 例 | 備考 |
|---|---|---|
| 対象（データ/画面/レイアウト等） | 「データ管理」「画面UI」「フィルタリング」 | **各対象に「設計書作成」→「実装」の順でSub-Issueを用意すること**（後述） |
| 機能単位 | 独立した機能ごとに分ける | 通常のスコープ分割 |
| フェーズ | 「Phase1: 現役のみ」「Phase2: OG拡大」等 | 依存関係を明記する |

**対象（データ/画面等）で分割する場合の必須ルール**: 「データ管理」「画面UI」のように対象でSub-Issueを分ける場合、各対象について**設計書作成を実装より前のSub-Issueとして必ず含めること**。実装Sub-Issueだけを並べて設計書作成を省略・後回しにしてはならない。

例: 「データ管理」を分割する場合 → 1. データ設計書の作成 → 2. データ管理の実装（1に依存）

設計書のない状態で実装を進めると、設計書という検証の拠り所がないまま実装が進み、実装ミスをその場で検出できなくなる（実例: メンバー関係性マップ機能でデータ設計書のないまま実装した結果、同ユニットエッジのラベルに `unit.id` を使う誤りが混入し、ユーザー報告まで発覚しなかった。#1260）。
...
**対象（データ/画面等）で分割する場合の例**（設計書作成を実装より前に配置する）:

```
分割案:
1. {対象}データ設計書の作成
   - 内容: {テーブル設計・API仕様・型定義等をまとめた設計書を作成}
   - ラベル: documentation
